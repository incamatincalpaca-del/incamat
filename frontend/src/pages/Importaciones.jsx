...
const columns = {
  ...
  MantenimientoSRequest: ["Solicitud", "Fecha y hora", "Descripción de la solicitud", "Empresa", "Estado"],
  MantenimientoHistorico: ["ID_Registro", "ID_Maquina", "Maquina", "Fecha", "Técnicos", "Tipo_Mantenimiento", "OT", "Código_Mantenimiento", "Duración", "Detalles de intervención", "Repuestos/Materiales", "Foto_evidencia", "Revisado"],
};
const label = (module) => module === "MantenimientoSRequest" ? "Mantenimientos (SRequest)" : module === "MantenimientoHistorico" ? "Historial de mantenimientos" : module;
...
function Importaciones() {
  ...
  return <DashboardLayout>
    <PageHeader eyebrow="CARGA MASIVA" title={`Importar ${label(module)}`} description={isHistory ? "Carga el historial técnico usando la misma estructura de los registros de mantenimiento de Incalpaca." : isSRequest ? "Plantilla oficial SRequest: carga solicitudes sin modificar sus encabezados." : "Usa las plantillas oficiales para crear o actualizar información sin duplicados."} />
    <main className="import-workspace">
      <section className="import-hero"><div><p className="eyebrow">MÓDULO DE CARGA</p><h2>Importación segura desde Excel</h2><p>{isHistory ? "El ID de registro identifica cada mantenimiento y evita duplicados al actualizar." : isSRequest ? "Incluye solicitud, fecha, urgencia, evidencia, área de origen y solicitante." : "La información se valida antes de guardar. Podrás ver errores y evitar registros duplicados."}</p></div><div className="import-hero-badges"><span>✓ Validación</span><span>✓ Actualización</span><span>✓ Historial</span></div></section>
      <section className="import-steps-grid">
        ...
        <article className="import-step"><span className="step-number">2</span><div><h2>Descarga y completa</h2><p>{isHistory ? "Registra una actividad por fila; no cambies el nombre de las columnas." : isSRequest ? "Usa el mismo diseño del archivo SRequest y no renombres sus columnas." : "Usa la plantilla para mantener el formato de Incalpaca."}</p><a className="button button-secondary" href={`${API}/importaciones/plantilla/${module}`}>Descargar plantilla Excel</a><small>Columnas: {columns[module].join(" · ")}</small></div></article>
        <article className="import-step import-drop"><span className="step-number">3</span><div><h2>Sube y valida</h2><p>{file ? file.name : "Selecciona el archivo Excel completado."}</p><button className="button button-primary" type="button" onClick={() => input.current?.click()}>{busy ? "Validando..." : "Elegir archivo Excel"}</button><input hidden ref={input} type="file" accept=".xlsx,.xls" onChange={choose} /><small>Excel .xlsx o .xls</small></div></article>
      </section>
