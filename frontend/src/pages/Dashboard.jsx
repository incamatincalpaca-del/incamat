  ...
  return <DashboardLayout>
    ...
    <section className="maintenance-overview">
      ...
      {historyView === "cards" ? <div className="maintenance-overview-grid history-type-grid">
        {summaryCards.map(([key, title, detail]) => <Link key={key} to={`/mantenimientos?historial=${key}`} className={`maintenance-summary-card ${key}`} title={`Ver máquinas y registros de ${title}`}><span>{title}</span><strong>{maintenanceSummary[key] || 0}</strong><small>{detail} · Ver detalle →</small></Link>)}
