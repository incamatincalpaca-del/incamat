...
router.get("/", async (_req, res) => { let conn; try { conn = await pool.getConnection(); const rows = await conn.query("SELECT id,nombre,usuario,correo,rol,estado,creado FROM usuarios ORDER BY nombre"); res.json(rows.map((row) => ({ ...row, estado: Boolean(row.estado) }))); } catch (_error) { res.status(500).json({ error: "No fue posible obtener los usuarios." }); } finally { if (conn) conn.release(); } });
...
router.patch("/:id", async (req, res) => {
  ...
  const fields = ["nombre=?", "correo=?", "rol=?", "estado=?"]; const values = [nombre.trim(), correo?.trim() || null, rol, estado ? 1 : 0];
